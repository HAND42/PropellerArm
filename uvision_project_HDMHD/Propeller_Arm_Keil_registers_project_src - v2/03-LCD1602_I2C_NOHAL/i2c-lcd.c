...
#define SLAVE_ADDRESS_LCD 0x4E // change this according to ur setup
  ...
  /*****************************************************************************************************************************************/
 /*		---------------------------------------------	I2C CONFIG PB6-PB9 - LCD COMMUNICATION		---------------------------------------*/
/*****************************************************************************************************************************************/
...
void I2C_Config(void)
{
	...
	*/
	...
	GPIOB->MODER |= (2 << 12) | (2 << 18);	 // Bits (13:12)= 1:0 --> Alternate Function for Pin PB6; Bits (19:18)= 1:0 --> Alternate Function for Pin PB9
	GPIOB->OTYPER |= (1 << 6) | (1 << 9);	 //  Bit8=1, Bit9=1  output open drain
	GPIOB->OSPEEDR |= (3 << 12) | (3 << 18); // Bits (13:12)= 0:1 --> Pull up for PIN PB6; Bits (19:18)= 1:1 --> High Speed for PIN PB9
	GPIOB->PUPDR |= (1 << 12) | (1 << 18);	 // Bits (13:12)= 0:1 --> Pull up for PIN PB6; Bits (19:18)= 0:1 --> pull up for PIN PB9
	GPIOB->AFR[1] |= (4 << 4);				 // Bits (27:26:25:24) = 0:1:0:0 --> AF4 for pin PB6;  Bits (7:6:5:4) = 0:1:0:0 --> AF4 for pin PB9
	GPIOB->AFR[0] |= (4 << 24);
...
void LCD_Write (uint8_t Address, uint8_t *Data, int size)
{
	...
	I2CSendSlaveAddress(Address);
	for (int i=0; i<size; i++) I2CWriteByte(*Data++);
	I2CStop ();
}
...
void lcd_init (void)
{
  ...
  // dislay initialisation
	...
	PrintConsole(WARNING,"Initialize the LCD, if it does print the Init done Press start we have to reset the mcu.");
	lcd_clear();
	lcd_put_cur (0,0);
