...
//		return MEMS_SUCCESS;
	...
	//TM_I2C_ReadMulti(I2C1, deviceAddr, Reg, Data, 1);  // 1 for single byte reading
	*Data = TM_I2C_Read(I2C1, deviceAddr, Reg);
...
u8_t WriteReg(u8_t deviceAddress, u8_t WriteAddr, u8_t Data) {
  ...
  //I2C_ByteWrite(&Data,  deviceAddress,  WriteAddr); 
	TM_I2C_Write(I2C1, deviceAddress, WriteAddr, Data);
...
status_t SetMode(Mode_t md) {
  ...
  case NORMAL:
    ...
    value2 &= 0xF7;
    value2 |= (MEMS_SET<<RHR);   //set HighResolution_BIT
    break;
  ...
  case LOW_POWER:		
    ...
    value |=  (MEMS_SET<<LPEN);
    value2 &= 0xF7;
    value2 |= (MEMS_RESET<<RHR); //reset HighResolution_BIT
    break;
