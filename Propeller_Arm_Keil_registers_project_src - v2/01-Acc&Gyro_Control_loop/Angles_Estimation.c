...
void Control_Loop_ThetaEstimation_CommandThrottle(void){
	...
	PWMd =  (((saturate_force_input(Fmoy + command*l1/I)/(1000*G))/0.17 +176.21)/0.17 ) / 10; // 	Divided by 10 because PWMd = 100 duty cycle = 1000us
	PWMg =  (((saturate_force_input(Fmoy - command*l1/I)/(1000*G))/0.17 +176.21)/0.17 ) / 10;
	MotorPWM_Set(PWMg, PWMd);
	SendValuesToQT(Get_Timestamp(), acc_data.theta_acc*180/M_PI,theta_data.theta*180/M_PI, PWMg, PWMd);
...
float saturate_force_input(float F){
	if (F> 121.29) return 121.29;
	else if(F<2.29) return 2.29;
	return F;
}
...
void TiltCompensatedThetaEstimation(void){
   ...
   if (isnan(acc_data.theta_acc) || isnan(gyro_data.gyro_y)) {
	...
	theta_data.pre_theta = theta_data.theta;
	}
}
...
void TiltCompensatedThetaEstimation2(void){
   ...
   if (isnan(acc_data.theta_acc) || isnan(gyro_data.gyro_y)) {
	...
	theta_data.pre_pre_theta = theta_data.theta;
	theta_data.pre_theta = theta_data.pre_theta;
